...
export default function GuildPage() {
    ...
    try {
      ...
      if (newChannels.length > 0) {
        ...
        if (first.type === "Text") {
          try {
            setMessages([]);
            const history = await invoke<GuildMessage[]>("get_messages", {
              channelId: first.id,
            });
            setMessages(history);
          } catch (e) {
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!cid) return;
    try {
      const msg = await invoke<GuildMessage>("send_message", {
        channelId: cid,
        content: text,
      });
  ...
  const refreshChannels = async () => {
    ...
    if (!gid) return;
    try {
      const newChannels = await invoke<Channel[]>("get_guild_channels", {
        id: gid,
      });
      setChannels(newChannels);
    } catch (e) {
      ...
      <div class="flex flex-col flex-1 min-w-0">
        <div class="md:hidden flex items-center justify-between gap-3 p-3 border-b border-surface0 bg-mantle min-w-0">
          ...
          <div class="flex-1 min-w-0 text-center">
            ...
            <div class="text-sm text-subtext0 truncate">
              # {activeChannelName()}
            </div>
          </div>
        ...
        >
          ...
          >
            <div class="flex items-center justify-between p-4 border-b border-surface0">
              <h2 class="font-bold text-subtext0 text-sm">
                {activeGuildName()}
              </h2>
